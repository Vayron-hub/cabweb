...
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    this.backendService.getUsuarios().subscribe({
      next: (usuarios: User[]) => {
        ...
        this.allUsers = usuarios.map(user => ({
          ...
          ultimoAcceso: (user as any).ultimoAcceso || (user as any).fechaUltimoAcceso || new Date(),
  ...
  isCustomer(): boolean { return this.userRole === 'Cliente'; }
  isAdmin(): boolean { return this.userRole === 'Admin'; }
  isSuperAdmin(): boolean { return this.userRole === 'SuperAdmin'; }
  isPrivileged(): boolean { return this.isAdmin() || this.isSuperAdmin(); }
