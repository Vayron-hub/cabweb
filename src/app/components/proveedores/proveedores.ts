...
export class ProveedoresComponent implements OnInit {
    ...
    if (this.searchTerm.trim()) {
      ...
      filtered = filtered.filter(
        (p) =>
          ...
          (Array.isArray(p.contacto) &&
            p.contacto.some((c) => c.toLowerCase().includes(term)))
      );
    }
  ...
  saveSupplier() {
    this.supplierFormData.contacto = (this.contactoInput || '')
      .split('\n')
      .map((l) => l.trim())
      .filter((l) => !!l);
    ...
    if (this.editingSupplier) {
      proveedorPayload.id = this.editingSupplier.id;
      this.backendService
        .updateProveedor(this.editingSupplier.id, proveedorPayload)
        .subscribe({
          next: () => {
  ...
  updatePagination() {
    this.totalPages = Math.ceil(
      this.filteredProveedores.length / this.itemsPerPage
    );
  ...
  getPaginatedProveedores(): Proveedor[] {
    ...
    return this.filteredProveedores.slice(
      startIndex,
  ...
  openEditSupplierModal(proveedor: Proveedor) {
    ...
    this.contactoInput = Array.isArray(proveedor.contacto)
      ? proveedor.contacto.join('\n')
      : '';
    this.showSupplierModal = true;
  }
  ...
  deleteSupplier(proveedor: Proveedor) {
    if (
      !confirm(`¿Está seguro de eliminar el proveedor "${proveedor.nombre}"?`)
    ) {
      return;
    }
  ...
  toggleSupplierStatus(proveedor: Proveedor) {
    ...
    this.backendService
      .updateProveedor(proveedor.id, proveedorPayload)
      .subscribe({
        next: () => {
  ...
  toggleSupplierSelection(proveedorId: number, event: any) {
    if (event.target.checked) {
      ...
      this.selectedProveedores = this.selectedProveedores.filter(
        (id) => id !== proveedorId
      );
    }
  }
  ...
  selectAllSuppliers(event: any) {
    if (event.target.checked) {
      this.selectedProveedores = this.getPaginatedProveedores().map(
        (p) => p.id
      );
    } else {
  ...
  allProductsSelected(): boolean {
    ...
    return (
      this.selectedProveedores.length === paginatedProveedores.length &&
  ...
  bulkDeleteSuppliers() {
    ...
    if (
      !confirm(
        `¿Está seguro de eliminar ${this.selectedProveedores.length} proveedores?`
      )
    ) {
      return;
    }
  ...
  // === TRACK BY ===
  trackByProveedorId(index: number, item: Proveedor) {
    return item.id;
  }
}
