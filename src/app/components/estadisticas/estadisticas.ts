...
export class EstadisticasComponent implements OnInit, OnDestroy {
  ...
  private loadZoneSpecificStatistics(zona: any) {
    ...
    // Intentar endpoint de estadísticas específicas por zona
    this.backendService.getEstadisticasZonaPorId(zona.id).subscribe({
      next: (estadisticas: any) => {
        console.log(`✅ Estadísticas específicas cargadas para ${zona.nombre}:`, estadisticas);
        // Combinar con estadísticas ya calculadas
        if (this.estadisticasGenerales) {
          this.estadisticasGenerales = { ...this.estadisticasGenerales, ...estadisticas };
        }
        this.cdr.detectChanges();
      },
      error: (error: any) => {
    ...
    // Intentar endpoint de tipos específicos por zona
    this.backendService.getEstadisticasTiposPorZona(zona.id).subscribe({
      next: (tipos: any[]) => {
        console.log(`✅ Tipos específicos cargados para ${zona.nombre}:`, tipos);
        // Reemplazar tipos calculados con datos del backend si existen
        if (tipos && tipos.length > 0) {
          this.estadisticasTipos = tipos;
          this.clasificacionesExitosas = tipos.map(t => t.cantidad);
        }
        this.cdr.detectChanges();
      },
      error: (error: any) => {
        console.log(`ℹ️ No hay tipos específicos para ${zona.nombre} (usando datos calculados)`);
        // No es un error crítico - los tipos ya están calculados desde detecciones
      }
    });

    // Intentar endpoint de horarios específicos por zona
    this.backendService.getEstadisticasHorariosPorZona(zona.id).subscribe({
      next: (horarios: any[]) => {
        console.log(`✅ Horarios específicos cargados para ${zona.nombre}:`, horarios);
        // Reemplazar horarios calculados con datos del backend si existen
      ...
      error: (error: any) => {
        console.log(`ℹ️ No hay horarios específicos para ${zona.nombre} (usando datos calculados)`);
  ...
  private generateMockStatistics() {
    ...
    this.estadisticasZonas = [
      { 
        ...
        totalDetecciones: Math.floor(zoneData.base * zoneData.multiplier), 
        porcentaje: 100 // 100% porque solo mostramos esta zona
      }
    ];
